...
class Agent:
    def __init__(
        ...
        '''
        :param model: model
        :param experience: Experience replay memory
        :param actions: ndarray with set of steps
        ...
        # create hallucination actions which contain the original actions and also the
        # original actions going in opposite z direction
        self.hallucinate_actions = np.concatenate((self.actions, self.actions), axis=0)
        self.hallucinate_actions[int(self.hallucinate_actions.shape[0] / 2):, 2] *= -1
    ...
    def choose_action(self, q, T=0.):
        q = np.copy(q)
        if np.any(self.actions[:, 2] < 0.0):
            neg_z_direction_action_idx = np.argmin(self.actions[:, 2])
            q[neg_z_direction_action_idx] = -5
        if T <= 1e-4:
            a = np.argmax(q)
        else:
            p = softmax(q / T)
            ...
            a = np.random.choice(np.arange(q.size), p=p.ravel())
        return a
    ...
    def perform_step(self):
        ...
        # in the following we add a batch dim to our state representation
        ...
        while True:
            # choose the action based on the combined advance values
            action_idx = self.choose_action(advance[0], self.params['action_temperature'])
            if (self.position + self.actions[action_idx])[2] > 0:
                break
    ...
    def hallucinate(self, n):
        ...
        # make imaginary steps
        for i in range(positions.shape[0]):
            n_imaginary_steps = np.random.randint(5)  # careful, randint is exclusive at the upper bound
            for _ in range(n_imaginary_steps):
                new_pos = positions[i] \
                    + self.hallucinate_actions[np.random.randint(len(self.hallucinate_actions))]
                # only make hallucinatory step if it doesn't lead to a rupture point
                if not np.isclose(new_pos[None, :], self.ruptures).all(axis=1).any():
                    positions[i] = new_pos
        s1 = positions
        a1 = np.random.randint(low=0, high=self.model.n_actions, size=s1.shape[0])
    ...
    def train_step(self):
        n_real_samples = round(self.params['batch_size'] * (1 - self.params['hallucinate']))
        n_hallucinated_samples = round(self.params['batch_size'] * self.params['hallucinate'])
        ...
        if n_real_samples > 0:
            # sample some states and hallucinate around them
        ...
        if n_hallucinated_samples > 0:
            # hallucinate
            hallucinated_s1, hallucinated_a1, hallucinated_r1, \
                    hallucinated_s2, hallucinated_t2 = self.hallucinate(n_hallucinated_samples)
            s1 = np.concatenate((s1, hallucinated_s1), axis=0)
            a1 = np.concatenate((a1, hallucinated_a1), axis=0)
            r1 = np.concatenate((r1, hallucinated_r1), axis=0)
            s2 = np.concatenate((s2, hallucinated_s2), axis=0)
            t2 = np.concatenate((t2, hallucinated_t2), axis=0)
        ...
        # q1  (needs to be computed after q2 for the backward pass to make sense!!)
        q1, _, _ = self.model(s1)
        q1 = q1[np.arange(q1.shape[0]), a1]

        # expected q2, using the value of q2 from the target network and
        # the probabilities of selecting the actions from the online network
    ...
    def end_of_episode_plots(self):
        ...
        # this is a bit hacky: we need to add fake measurements to the grid, but
        # only if the Model expects measurements features
        if self.params['n_features'] == 5:
            grid = np.concatenate(
                (grid, np.zeros((grid.shape[0], 2))), axis=2)
